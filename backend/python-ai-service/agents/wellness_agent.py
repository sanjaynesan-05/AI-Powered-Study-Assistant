...
def generate_wellness_assessment(request_data: Dict[str, Any]) -> Dict[str, Any]:
    """Main function expected by Flask server for wellness assessment."""
    # Extract parameters from request
    stress_level = request_data.get('stress_level', 5)
    hours_studied = request_data.get('hours_studied', 2)
    break_frequency = request_data.get('break_frequency', 'medium')
    physical_symptoms = request_data.get('physical_symptoms', [])
    
    agent = WellnessAgent()
    facial_data = request_data.get('facial_data', {})
    activity_data = {
...
def get_wellness_assessment(facial_data: Dict = None, activity_data: Dict = None) -> Dict[str, Any]:
    """Legacy helper function to get wellness assessment."""
    agent = WellnessAgent()
    return agent.assess_wellness(facial_data, activity_data)
