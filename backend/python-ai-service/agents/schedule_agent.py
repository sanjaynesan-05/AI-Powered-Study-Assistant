...
def optimize_schedule(request_data: Dict[str, Any]) -> Dict[str, Any]:
    """Main function expected by Flask server for schedule optimization."""
    # Extract parameters from request
    subjects = request_data.get('subjects', ['General Study'])
    available_hours = request_data.get('available_hours', 8)
    priorities = request_data.get('priorities', [])
    deadlines = request_data.get('deadlines', [])
    
    # Create mock learning data
    learning_data = {
        "resources": [{"title": f"{subject} study materials", "estimated_time": 2} for subject in subjects],
        "total_estimated_time": len(subjects) * 2
    }
    
    # Create mock wellness data
    wellness_data = {
        "energy_level": 0.7,
        "stress_level": 0.3,
        "preferred_break_frequency": 60
    }
    
    # Use the first subject as main topic
    main_topic = subjects[0] if subjects else "General Study"
    
    return get_study_plan(main_topic, learning_data, wellness_data)
