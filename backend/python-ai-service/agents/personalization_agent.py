...
def get_personalization(request_data: Dict[str, Any]) -> Dict[str, Any]:
    """Main function expected by Flask server for personalization."""
    # Extract parameters from request
    student_id = request_data.get('student_id', 'demo_student')
    topic = request_data.get('topic', 'general learning')
    past_performance = request_data.get('past_performance', [])
    preferences = request_data.get('preferences', {})
    
    # Create mock learning resources
    learning_resources = {
        "resources": [
            {"title": f"{topic} basics", "type": "article", "difficulty": "beginner"},
            {"title": f"{topic} advanced", "type": "video", "difficulty": "advanced"}
        ],
        "difficulty": "intermediate"
    }
    
    # Create mock wellness assessment
    wellness_assessment = {
        "energy_level": 0.7,
        "stress_level": 0.3,
        "focus_level": 0.8
    ...
    return get_personalized_path(topic, learning_resources, wellness_assessment, student_id, past_performance)
